    ...
    if args.sort:
        if args.sort in df.columns:
            ...
            def parse_finviz_metric(val):
                ...
                if s == '-': return float('inf') if args.asc else -float('inf') # Push missing data to the bottom
                
                mult = 1.0
                if s.endswith('B'): mult = 1e9; s = s[:-1]
                elif s.endswith('M'): mult = 1e6; s = s[:-1]
                elif s.endswith('K'): mult = 1e3; s = s[:-1]
                elif s.endswith('%'): mult = 0.01; s = s[:-1]
            ...
            print(f"Warning: Sort column '{args.sort}' not found. Available columns: {', '.join(df.columns)}")
